// POZNÁMKA: admin-inbox.astro a admin/codes.astro používajú server-side autentifikáciu.
// Tento súbor slúži len na lokálnu správu recenzií cez localStorage (review queue).
...
function bindLoginForm() {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    // Overenie hesla cez server (nevystavuje heslo v JS kóde)
    try {
      ...
      const res = await fetch('/admin/codes', {
        method: 'POST',
        body: formData,
        redirect: 'manual' // nezobrazuj redirect
      });

      // Ak server odpovedal redirect (302) alebo OK, login bol úspešný
      ...
      message.textContent = 'Chyba pri prihlasovaní.';
    }
  });
}
...
function renderQueues() {
  ...
  if (!pending.length) {
    pendingList.appendChild(createEmpty('Zatiaľ žiadne čakajúce recenzie.'));
  } else {
  ...
  if (!approved.length) {
    approvedList.appendChild(createEmpty('Zatiaľ žiadne schválené recenzie.'));
  } else {
...
function createApprovedRow(review) {
  ...
  unpublish.textContent = 'Stiahnuť z webu';
...
function createBaseRow(review) {
  ...
  code.textContent = `Kód: ${review.projectCode}`;
...
function approveReview(id) {
  ...
  writeQueue(REVIEWS_PENDING_KEY, pending.filter((item) => item.id !== id));
...
function unpublishReview(id) {
  const approved = readQueue(REVIEWS_APPROVED_KEY);
  writeQueue(REVIEWS_APPROVED_KEY, approved.filter((item) => item.id !== id));
  renderQueues();
}
